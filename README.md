...
Built with [Astro](https://astro.build) (SSR) + Tailwind, and deployed on
**Cloudflare** — so all server code runs on the Web platform (Web Crypto, `fetch`),
with no Node-only dependencies. It's plain Astro: no UI framework, just a small
client `<script>` for the form's interactivity.
...
| UI             | Astro components + Tailwind v4 (no UI framework)    |
...
- `src/middleware.js` gates the form page `/` (redirect to Slack sign-in) and the
  `/api/submit` endpoint (401 without a session). `/docs` is public.
